...
### 2，接线调试
将上位机的uart 接入驱动板的 uart 接口（只需接 3 根线,RX,TX,GND,12V接到副板 UART 插座的 12V），轮毂电机按 UVW 与霍尔接好，将开关（**请使用自复位开关**）接入开关插座（主板红色的 2Pin 插座），这样主副板就可以跟主板同步开关机了。  
开机后就可以测试了，**测试时请悬空轮毂电机**，已经写好的测试脚本在 MM32SPIN05_BOARD_BLDC_DRIVER 中 py 中，使用 python3 执行即可，板子的状况会通过 uart 传过来，给 uart 输入转速即可测试。
